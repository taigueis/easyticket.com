    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            if (response.existemViagens) {
                ...
                if (linha_autocarro === '3001' && response.num_registos >= 68) {
                    // Não é possível adicionar mais registos para a linha 3001
                    var errorText = document.querySelector('.error__text');
                    errorText.innerHTML = "<span>Não é possível adicionar mais trajetos para esta linha!</span>";
                    errorText.style.display = 'block';
                    document.querySelector('.error').style.display = 'block';
                    // Chamar a função para rolar de volta para o topo da página
                    backToTop();
